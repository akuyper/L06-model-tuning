...
::: {.callout-important collapse="true"}
## Load Package(s) & Needed Objects

Recall that it is best practice to load your packages towards the top of your document.

Students should set the seed directly before any random process (for example data splitting and fitting random forest models) and make a comment/note at the top of any R script that alerts potential users that a random process is being used.

All necessary objects (split data, tuned/fit objects, etc.) should be loaded into the qmd at the top of the document, meaning no random process should be happening directly within this document.
:::
...
1.  An elastic net model with the `glm` engine (tune `penalty` and `mixture`);
2.  A $k$-nearest neighbors model with the `kknn` engine (tune `neighbors`);
3.  A boosted tree model with the `xgboost` engine (tune `mtry`, `min_n`, `learn_rate`, and `trees`).
...
For elastic net and knn, setup and store a regular grid with 5 levels of possible values for each hyperparameter we identified for tuning.

For boosted tree, use a grid with 5 levels for `mtry`, `min_n`, and `learn_rate` and 3 levels for `trees`.
...
-   For `penalty`, use `update()` to set `range = c(-5, -0.2)`.

-   For `mixture`, use `update()` to set `range = c(0, 1)`.

-   For `mtry`, use `update()` (as shown above) to set the upper limit to an appropriate value.

-   For `learn_rate`, use `update()` to set `range = c(-5, -0.2)`.

-   For `trees`, use `update()` to set `range = c(500, 1500)`.
...
Build a table that provides the mean RMSE, its standard error, and n (number of times RMSE we estimated) for the best model per model type. Which model type produced the best model? Explain how you made your choice. From the first part of the Task you should be able to identify this model's hyperparameter value(s). 
